...
_VRSOUND_ROOT = Path(__file__).resolve().parents[1]
_DEFAULT_EXT_WEIGHTS_BASE = _VRSOUND_ROOT / 'weights' / 'ext_weights'
EXT_WEIGHTS_BASE = Path(
    os.environ.get('FOLEY_OMNI_EXT_WEIGHTS', str(_DEFAULT_EXT_WEIGHTS_BASE))
)
...
class Runner:
    ...
    def __init__(self,
        ...
        if cfg.model.endswith('16k'):
            ...
            mode = '16k'
        elif cfg.model.endswith('44k') or cfg.model.endswith('44k_v2'):
        ...
        empty_string_path = EXT_WEIGHTS_BASE / 'empty_string.pth'
        empty_string_feat = torch.load(empty_string_path, weights_only=True)[0]
    ...
    def eval(self, audio_dir: Path, it: int, data_cfg: DictConfig) -> dict[str, float]:
        # 评估代码已注释，因为缺少 av_bench 依赖
        self.log.info(f'Evaluation skipped (av_bench not available)')
        output_metrics = {}
