...
def setup_training_datasets(cfg: DictConfig) -> tuple[Dataset, DistributedSampler, DataLoader]:
    ...
    if cfg.example_train:
        # unbal_audio = load_audio_data(cfg, cfg.data.unbal_audioset)
        unbal_audio_pq7_label = load_audio_data(cfg, cfg.data.unbal_audioset_pq7_label)
        # dataset = MultiModalDataset([],[unbal_audio_pq7_label])
        # unbal_audio_pq7_sample_label = load_audio_data(cfg, cfg.data.unbal_audioset_pq7_sample_label)
        # bal_audio_label = load_audio_data(cfg, cfg.data.bal_audioset_label)
        # bal_audio_short = load_audio_data(cfg, cfg.data.bal_audioset_short)
        kling = load_vgg_data(cfg, cfg.data.kling)
        dataset = MultiModalDataset([kling], [unbal_audio_pq7_label])
        # vgg_IB30 = load_vgg_data(cfg, cfg.data.vgg_IB30_train)
        # vgg_IB30_smd_label = load_vgg_data(cfg, cfg.data.vgg_IB30_smd_train)
        # vgg_IB30_smd_short = load_vgg_data(cfg, cfg.data.vgg_IB30_smd_short_train)
        # vgg_IB30_smd_long = load_vgg_data(cfg, cfg.data.vgg_IB30_smd_long_train)
        # youtube_smd_short = load_vgg_data(cfg, cfg.data.youtubegaming_short_train)
        # dataset = MultiModalDataset([vgg_IB30_smd_label, vgg_IB30_smd_short,youtube_smd_short],[unbal_audio_pq7,bal_audio_label,bal_audio_short])
        # dataset = MultiModalDataset([vgg_IB30_smd_label, vgg_IB30_smd_short,youtube_smd_short],[])
    else:
