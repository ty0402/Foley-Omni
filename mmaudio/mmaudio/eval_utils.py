...
_MMAUDIO_ROOT = Path(__file__).resolve().parents[1]
_PUBLIC_REPO_ROOT = _MMAUDIO_ROOT.parent
_DEFAULT_EXT_WEIGHTS_CANDIDATES = [
    _PUBLIC_REPO_ROOT / 'ckpts' / 'mmaudio' / 'ext_weights',
    _MMAUDIO_ROOT / 'weights' / 'ext_weights',
]
_DEFAULT_MODEL_WEIGHTS_BASE = _MMAUDIO_ROOT / 'weights'
...
EXT_WEIGHTS_BASE = Path(
    os.environ.get('FOLEY_OMNI_EXT_WEIGHTS', str(_first_existing_dir(_DEFAULT_EXT_WEIGHTS_CANDIDATES)))
)
MODEL_WEIGHTS_BASE = Path(
    os.environ.get('FOLEY_OMNI_MMAUDIO_WEIGHTS', str(_DEFAULT_MODEL_WEIGHTS_BASE))
)
...
class ModelConfig:
    ...
    mode: str
    synchformer_ckpt: Path = EXT_WEIGHTS_BASE / 'synchformer_state_dict.pth'
...
small_16k = ModelConfig(model_name='small_16k',
                        model_path=MODEL_WEIGHTS_BASE / 'mmaudio_small_16k.pth',
                        vae_path=EXT_WEIGHTS_BASE / 'v1-16.pth',
                        bigvgan_16k_path=EXT_WEIGHTS_BASE / 'best_netG.pt',
                        mode='16k')
small_44k = ModelConfig(model_name='small_44k',
                        model_path=MODEL_WEIGHTS_BASE / 'mmaudio_small_44k.pth',
                        vae_path=EXT_WEIGHTS_BASE / 'v1-44.pth',
                        bigvgan_16k_path=None,
                        mode='44k')
medium_44k = ModelConfig(model_name='medium_44k',
                         model_path=MODEL_WEIGHTS_BASE / 'mmaudio_medium_44k.pth',
                         vae_path=EXT_WEIGHTS_BASE / 'v1-44.pth',
                         bigvgan_16k_path=None,
                         mode='44k')
large_44k = ModelConfig(model_name='large_44k',
                        model_path=MODEL_WEIGHTS_BASE / 'mmaudio_large_44k.pth',
                        vae_path=EXT_WEIGHTS_BASE / 'v1-44.pth',
                        bigvgan_16k_path=None,
                        mode='44k')
large_44k_v2 = ModelConfig(model_name='large_44k_v2',
                           model_path=MODEL_WEIGHTS_BASE / 'mmaudio_large_44k_v2.pth',
                           vae_path=EXT_WEIGHTS_BASE / 'v1-44.pth',
                           bigvgan_16k_path=None,
                           mode='44k')
