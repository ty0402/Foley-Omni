...
def save_video(
    ...
    # Create video clip
    clip = ImageSequenceClip(frames, fps=fps_value)
    ...
    try:
        ...
        final_clip.fps = fps_value
    logging.info(
        "[save_video] Using fps=%.2f, clip.fps=%s",
        fps_value,
        getattr(final_clip, "fps", None),
    )

    final_clip.write_videofile(
        output_path, codec="libx264", audio_codec="aac", fps=fps_value, verbose=False, logger=None
    )
    final_clip.close()
